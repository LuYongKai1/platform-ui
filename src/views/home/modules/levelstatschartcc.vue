...
import { NSelect, NSpin, NSlider } from 'naive-ui';
...
// 添加时间区间选择相关状态
const timeRangeValue = ref<[number, number]>([0, 0]);
const availableTimePoints = ref<string[]>([]);
const currentSelectedTime = ref<string>('');
const isUpdatingTimeRange = ref(false);

// 计算属性：开始和结束时间索引
const startTimeIndex = computed(() => timeRangeValue.value[0]);
const endTimeIndex = computed(() => timeRangeValue.value[1]);
    ...
    if (data.levels && Array.isArray(data.levels)) {
      ...
      if (timePoints.length === 0) {
        ...
        return;
            ...
            // 确保选择的时间索引有效
      let newStartIndex = timeRangeValue.value[0];
      let newEndIndex = timeRangeValue.value[1];

      if (newStartIndex >= timePoints.length) {
        newStartIndex = timePoints.length - 1;
      }
      if (newEndIndex >= timePoints.length) {
        newEndIndex = timePoints.length - 1;
      }

      // 如果还没有初始化，根据时间范围模式设置默认区间
      if (newEndIndex === 0 && timePoints.length > 1) {
        if (timeRange.value === '24h') {
          // 24小时模式：从当前时间往前推24小时
          const twentyFourHoursAgoIndex = calculate24HoursAgoIndex();
          newStartIndex = twentyFourHoursAgoIndex;
          newEndIndex = timePoints.length - 1;
        } else {
          // 1小时模式：显示最近60个数据点或最近5个时间点
          const recentCount = Math.min(60, Math.max(5, timePoints.length));
          newStartIndex = Math.max(0, timePoints.length - recentCount);
          newEndIndex = timePoints.length - 1;
        }
      }
      ...
      // 更新时间范围值，避免触发watch
      if (newStartIndex !== timeRangeValue.value[0] || newEndIndex !== timeRangeValue.value[1]) {
        isUpdatingTimeRange.value = true;
        timeRangeValue.value = [newStartIndex, newEndIndex];
        nextTick(() => {
          isUpdatingTimeRange.value = false;
        });
      }
...
// 添加时间区间选择的监听器
watch(timeRangeValue, () => {
  // 只有在非程序性更新时才重新处理图表数据
  if (!isUpdatingTimeRange.value) {
    nextTick(() => {
      processChartData();
    });
  }
...
// 计算24小时前的时间索引
// 更加稳健地计算24小时前的索引
const calculate24HoursAgoIndex = (): number => {
  if (availableTimePoints.value.length === 0) return 0;

  const latestTimeStr = availableTimePoints.value.at(-1);
  if (!latestTimeStr) return 0;

  try {
    const latestDate = new Date(latestTimeStr);
    const targetTime = new Date(latestDate.getTime() - 24 * 60 * 60 * 1000);

    for (let i = 0; i < availableTimePoints.value.length; i++) {
      const current = new Date(availableTimePoints.value[i]);
    ...
    // fallback: 1440 点，向后回推
    const total = availableTimePoints.value.length;
    return total > 1440 ? total - 1440 : 0;
  }
};
...
const toggleTimeRange = (range: '1h' | '24h') => {
  ...
  // 根据选择的时间范围，自动调整时间区间
  if (range === '24h' && availableTimePoints.value.length > 0) {
  const twentyFourHoursAgoIndex = calculate24HoursAgoIndex();
  isUpdatingTimeRange.value = true;
  timeRangeValue.value = [twentyFourHoursAgoIndex, availableTimePoints.value.length - 1];

  nextTick(() => {
    isUpdatingTimeRange.value = false;
    processChartData(); // ⚠️ 注意，这里必须放 nextTick 后
  });
}
};
...
const selectPreviousTime = () => {
  const [start, end] = timeRangeValue.value;
  if (start > 0) {
    const newStart = start - 1;
    // 如果开始时间超过了结束时间，也调整结束时间
    const newEnd = newStart > end ? newStart : end;
    timeRangeValue.value = [newStart, newEnd];
  }
};

const selectNextTime = () => {
  const [start, end] = timeRangeValue.value;
  if (end < availableTimePoints.value.length - 1) {
    const newEnd = end + 1;
    // 如果结束时间超前太多，也调整开始时间
    const newStart = newEnd - start > 10 ? Math.max(0, newEnd - 10) : start;
    timeRangeValue.value = [newStart, newEnd];
  }
};
</script>
