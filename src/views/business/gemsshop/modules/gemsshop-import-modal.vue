...
async function downloadTemplate() {
  ...
  try {
    const response = await fetchDownloadGemsShopTemplate({
    ...
    window.$message?.success(
      $t("page.manage.products.importModal.templateDownloadSuccess")
    );
  } catch (error: any) {
...
function downloadErrorFile(base64Data: string) {
  try {
    ...
    // @ts-ignore
    window.$message?.info(
      $t("page.manage.products.importModal.errorFileDownloadSuccess")
    );
  } catch (error) {
...
async function confirmImport() {
  ...
  try {
    ...
    // 调用导入商品API
    const response = await fetchImportGemsShop(formData);

    // 使用通用错误处理函数检查响应
    if (handleApiResponseError(response, "导入商品")) {
      // 检查是否有错误文件需要下载
      const errorFileBase64 = (response as any)?.response?.data
        ?.errorFileBase64;
      if (errorFileBase64) {
